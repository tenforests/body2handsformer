...
def main(args):
    ...
    device = torch.device(args.cuda if torch.cuda.is_available() else 'cpu') #device=cpu
    args.device = device
...
#####################################帧率变化###########
def frameCaculate(array_file,array_hand,array_body,array_img,frame_num=32):
...
def load_data(args, rng):
    ...
    for key, value in DATA_PATHS.items():
        ...
        if gt_windows is None:
            if args.require_image:
                hand_ims = curr_p0[1]   #curr_p0[0] body的关节点 curr_p0[1]手部resnet
                curr_p0 = curr_p0[0]
            ...
            p0_paths = np.concatenate((p0_paths, curr_paths), axis=0)             # p0_path 输入文件数据
    if args.seq_length!=64:
        p0_paths,quant_windows,gt_windows,hand_ims=frameCaculate(p0_paths,quant_windows,gt_windows,hand_ims,args.seq_length)
    ...
    train_X, test_X = gt_windows[train_idx, :, :], gt_windows[test_idx, :, :]     #  X body 
    train_Y, test_Y = quant_windows[train_idx, :, :], quant_windows[test_idx, :, :]  #  Y hand
    if args.require_image:
    ...
    args.Mean_Y,args.Std_Y=body_mean_Y,body_std_Y
    ...
    for bii, bi in enumerate(batchinds):
        ...
        with torch.no_grad():
            mpjre, mpjpe = evaluate_prediction(output, outputGT, 
                                                ...
                                                mean=torch.from_numpy(args.Mean_Y).to(device), std=torch.from_numpy(args.Std_Y).to(device),device=args.device)
        total_mpjre += mpjre 
        total_mpjpe += mpjpe 
