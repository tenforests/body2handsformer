...
class ST_former(nn.Module):
	...
	def build_net(self):
		...
		self.S_pos_embed = nn.Parameter(torch.randn(6,self.s_out_dim))
		# 定义编码层
	...
	def Spatial_forward(self,body_input):	 # 每次对(n*f)的数据并行处理  
		# n*f*(1024)   n*f*(6*6)
		body_input = rearrange(body_input, 'n f (w c) ->(n f) w c', w=6)      # (n*f)*6*6
		res = self.S_input_embdding(body_input)                            # (n*f)*6*64
		...
		res = res.mean(1) # ((n*f)*1*512)								 # (n*f)*(joint+1)*64 -> (n*f)*64
		# print(res.shape)
