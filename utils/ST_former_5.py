...
class ST_former(nn.Module):
	...
	def build_net(self):
		...
		#class_token的定义
		self.class_token = nn.Parameter(torch.zeros(1,self.s_out_dim))
		# 定义可学习的位置编码
		self.T_pos_embed = nn.Parameter(torch.zeros(self.seq_length, self.t_out_dim))
		self.S_pos_embed = nn.Parameter(torch.zeros(8,self.s_out_dim))
		# 定义编码层
		...
		self.regression_head = nn.Sequential(
            ...
            nn.Linear(self.s_out_dim*8 , (self.s_out_dim*8)*2),
	    	nn.LeakyReLU(0.2, True),
		    nn.LayerNorm((self.s_out_dim*8)*2),
		    nn.Dropout(self.dropout),
            nn.Linear((self.s_out_dim*8)*2,self.s_out_dim*8),
	    	nn.LeakyReLU(0.2, True),
		    nn.LayerNorm(self.s_out_dim*8),
		    nn.Dropout(self.dropout),
            nn.Linear(self.s_out_dim*8,252)
			)
	...
	def Spatial_forward(self,hand_input,body_input):	 # 每次对(n*f)的数据并行处理  
		...
		res = torch.concat((y,x),dim=1)           	     # (n*f)*9*64
		res = res.add(self.S_pos_embed)                       			 # 加入位置编码
		res = self.S_transformer(res)									 # 送入S_transformer
		#res = np.mean(res,axis=1) # ((n*f)*1*512)								 # (n*f)*(joint+1)*64 -> (n*f)*64
	...
	def forward(self,hand_input,body_input):                              # n*f*(1024)   n*f*(6*6)
		...
		_out = self.regression_head(s_out)                            	  #  n*f*(42*6)
